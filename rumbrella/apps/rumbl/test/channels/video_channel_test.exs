defmodule Rumbl.Channels.VideoChannelTest do
  ...
  test "inserting annotations", %{socket: socket, video: vid} do
  ...
  test "new annotations trigger InfoSys", %{socket: socket, video: vid} do
    ...
    {:ok, _, socket} = subscribe_and_join(socket, "videos:#{vid.id}", %{})
    ref = push socket, "new_annotation", %{body: "1+1?", at: 321}
    assert_reply ref, :ok, %{}
    assert_broadcast "new_annotation", %{body: "1+1?", at: 321}
    assert_broadcast "new_annotation", %{body: "2", at: 321}
  end
end
